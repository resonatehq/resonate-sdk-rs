...
/// Thread-safe map of application state keyed by type.
/// Set at startup via `with_state()`, shared across all contexts.
pub(crate) type StateMap = Arc<parking_lot::RwLock<HashMap<TypeId, Arc<dyn Any + Send + Sync>>>>;
...
pub struct Resonate {
    ...
    // Application state (set via with_state(), accessed via ctx.state::<T>())
    state: StateMap,
}
...
impl Resonate {
    ...
    pub fn new(config: ResonateConfig) -> Self {
        ...
        let state: StateMap = Arc::new(parking_lot::RwLock::new(HashMap::new()));
    ...
    /// Store application state accessible to durable functions via `ctx.state::<T>()`.
    /// Call before `register()`. Multiple types can be stored.
    ///
    /// State is a runtime resource (e.g. database connections) — never serialized.
    /// On process restart, provide fresh state via `with_state()`.
    pub fn with_state<T: Send + Sync + 'static>(self, value: T) -> Self {
        self.state.write().insert(TypeId::of::<T>(), Arc::new(value));
        self
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn with_state_stores_value() {
        let r = Resonate::local().with_state(42_i32).with_state("hello".to_string());

        let map = r.state.read();
        assert!(map.contains_key(&TypeId::of::<i32>()));
        assert!(map.contains_key(&TypeId::of::<String>()));
    }

    #[tokio::test]
    async fn with_state_multiple_types() {
        struct DbConn(String);
        struct Config(u32);

        let r = Resonate::local()
            .with_state(DbConn("localhost".to_string()))
            .with_state(Config(8080));

        let map = r.state.read();
        assert_eq!(map.len(), 2);
        let db = map.get(&TypeId::of::<DbConn>()).unwrap().clone().downcast::<DbConn>().unwrap();
        assert_eq!(db.0, "localhost");
        let cfg = map.get(&TypeId::of::<Config>()).unwrap().clone().downcast::<Config>().unwrap();
        assert_eq!(cfg.0, 8080);
    }

    #[tokio::test]
    async fn with_state_overwrites_same_type() {
        let r = Resonate::local()
            .with_state(1_i32)
            .with_state(2_i32);

        let map = r.state.read();
        let val = map.get(&TypeId::of::<i32>()).unwrap().clone().downcast::<i32>().unwrap();
        assert_eq!(*val, 2);
    }

    #[test]
    #[should_panic(expected = "not registered")]
    fn context_state_panics_on_missing_type() {
        let harness = crate::test_utils::TestHarness::new();
        let effects = harness.build_effects(vec![]);
        let ctx = crate::test_utils::test_context("panic-test", effects);

        ctx.state::<String>(); // Should panic — String not registered
    }
}
