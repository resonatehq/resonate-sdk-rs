//! Vendored verbatim from `resonate/src/types.rs` (the Resonate server).
//! Only change: module-level `#![allow(dead_code)]` since the SqliteNetwork
//! does not exercise every server response type.
#![allow(dead_code)]
